...
def buscar_qualidade_no_texto(texto_normalizado: str) -> tuple[float | None, int | None]:
    ...
    """
    def afirmada(expressao: str) -> bool:
        for match in re.finditer(r"\b" + re.escape(expressao) + r"\b", texto_normalizado):
            previous = re.findall(r"\w+", texto_normalizado[:match.start()])[-5:]
            if not any(token in {"nao", "sem", "nunca", "nem"} for token in previous):
                return True
        return False

    if any(afirmada(value) for value in PALAVRAS_QUALIDADE_MUITO_ALTA):
        return 8.0, 200
    if any(afirmada(value) for value in PALAVRAS_QUALIDADE_ALTA):
        return 7.0, 100
    return None, None
